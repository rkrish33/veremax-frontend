...
@Component({
  selector: "app-vehicle-tracker-report",
  templateUrl: "./vehicle-tracker-report.component.html",
  styleUrls: ["./vehicle-tracker-report.component.scss"],
})
export class VehicleTrackerReportComponent implements OnInit {
  submit: boolean;
  listvehicles;
  count;
  limit;
  ...
  async clusters($event = "") {
    let res = await this.vehicle.cluster({
      like: $event,
  ...
  async listVehicle() {
    let res = await this.vehicle.trackkm({
  ...
  async download() {
    ...
    if (this.count1) {
      ...
      for (var i = 0; i < temp.length; i++) {
        ...
        param["Start Km"] = temp[i]["one_open"];
        param["Stop Km"] = temp[i]["one_close"];
        param["Running Kms"] = temp[i]["one_km"];
